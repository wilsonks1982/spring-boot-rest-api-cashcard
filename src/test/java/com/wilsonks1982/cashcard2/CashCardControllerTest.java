...
public class CashCardControllerTest {
    ...
    class GetSingleCashCardTests {
        ...
        void shouldRetrieveExistingCashCard() {
            ...
            assertThat(getResponse.getBody().owner()).isEqualTo(BASIC_AUTH_USER1);
        ...
        void shouldReturnForbiddenForAnotherUsersCard() {
            ...
            log.info("Created card with id: {} for user: {}", createResponse.getBody().id(), BASIC_AUTH_USER1);
            Long cardId = createResponse.getBody().id();
        ...
        void shouldReturnServerPopulatedOwner() {
            ...
            // Assert
            assertThat(getResponse.getBody().owner()).isEqualTo("user1");
    ...
    class CreateCashCardTests {
        ...
        void shouldCreateCashCardWithValidData() {
            ...
            assertThat(response.getBody().owner()).isEqualTo(BASIC_AUTH_USER1);
    ...
    @Nested
    @DisplayName("GET /cashcard/search - Search CashCard by Card Number")
    class SearchCashCardByCardNumberTests {
        @Test
        @DisplayName("Should retrieve CashCard by valid card number")
        void shouldRetrieveCashCardByValidCardNumber() {
            // Arrange - Create a card
            String cardNumber = "1234567890";
            ResponseEntity<CashCard> createResponse = restTemplate
                    .withBasicAuth(BASIC_AUTH_USER1, BASIC_AUTH_PASSWORD1)
                    .postForEntity(API_BASE_URL,
                            createCashCardRequest(cardNumber, BigDecimal.valueOf(100.00)),
                            CashCard.class);

            log.info("Created card with card number: {} for search test", cardNumber);

            // Act - Search by card number
            ResponseEntity<CashCard> searchResponse = restTemplate
                    .withBasicAuth(BASIC_AUTH_USER1, BASIC_AUTH_PASSWORD1)
                    .getForEntity(API_BASE_URL + "/search?cardNumber=" + cardNumber, CashCard.class);

            // Assert
            assertThat(searchResponse.getStatusCode()).isEqualTo(HttpStatus.OK);
            assertThat(searchResponse.getBody()).isNotNull();
            assertThat(searchResponse.getBody().cardNumber()).isEqualTo(cardNumber);
            log.info("Successfully retrieved CashCard by card number: {}", cardNumber);
        }

        // Additional tests for invalid card number format, non-existent card number, etc. can be added here
    }
}
